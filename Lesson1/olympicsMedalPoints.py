...
def points():
    ...
    # YOUR CODE HERE
    gold_points = np.dot(gold,4)
    silver_points = np.dot(silver,2)
    bronze_points = np.dot(bronze,1)

    olympic_points_df = {
        'country_name' : Series(countries),
        'points': Series(gold_points + silver_points + bronze_points)
    }

    # print DataFrame(olympic_points_df)
